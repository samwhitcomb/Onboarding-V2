...
export const CourseDetailModal: React.FC<CourseDetailModalProps> = ({ 
  ...
  const handlePlayLaterClick = (e: React.MouseEvent) => {
    e.stopPropagation()
  ...
  const handleTeeOffClick = (e: React.MouseEvent) => {
    e.stopPropagation()
  ...
  const getVideoUrl = () => {
    ...
    if (videoMap[course.id]) {
      ...
      return `${basePath}videos/${videoMap[course.id]}`
    }
    
    // Fallback to course videoUrl or default pattern
    if (course.videoUrl) {
      const basePath = import.meta.env.BASE_URL || '/'
      return `${basePath}videos/${course.videoUrl}`
    }
  ...
  const handleVideoClick = (e: React.MouseEvent) => {
    ...
    if (video.tagName !== 'VIDEO') {
      ...
      const heroSection = (e.currentTarget as HTMLElement).closest('.modal-hero')
      video = heroSection?.querySelector('.modal-hero-video') as HTMLVideoElement
    }

...
export function getBlurb(course: Course): string[] {
  ...
  const templates = [
    (c: Course) => [
      ...
      `Known for its character and consistency, it has earned a ${c.rating?.toFixed ? c.rating.toFixed(1) : c.rating || 0}/5 reputation. Regulars point to its history and memorable holes as the reason it remains a must-play in the region.`
    ],
    (c: Course) => [
      ...
      `Its acclaim (${c.rating?.toFixed ? c.rating.toFixed(1) : c.rating || 0}/5) comes from a mix of architecture, conditioning, and a few signature moments that keep players talking long after the round.`
    ],
    (c: Course) => [
      `${c.name} sits in ${c.location}, offering a classic feel with enough bite to keep low-handicaps honest. ${c.description || ''}`,
      `Stories around the clubhouse often cite its storied holes and steady challenge; with a ${c.rating?.toFixed ? c.rating.toFixed(1) : c.rating || 0}/5 rating, it stays firmly on the shortlist for return trips.`
    ],
  ]
